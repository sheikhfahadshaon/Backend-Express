...
const { signUpSchema, loginSchema, updatePasswordSchema } = require('../validator/userValidator');
...
userRoutes.delete("/", verifyToken, async (req, res) => {
    try {
        ...
        res.clearCookie('token');
        res.status(200).json({ "message": "Your profile is deleted successfully", "note": deletedUser });
    } catch (err) {
        console.log(err);
        console.log("error from userroutes delete");
